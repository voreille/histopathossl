...
class MultiHeadContrastiveLoss(BaseContrastiveLoss):
    ...
    def loss_neighbor(self, q, k_neighbors, queue, valid_neighbors_count):
        ...
        l_pos_neighbors = l_pos_neighbors.sum(
            dim=1,
            keepdim=True) / valid_neighbors_count.unsqueeze(1).clamp(min=1)
        ...
        # Ground truth labels (positive key is at index 0)
        labels = torch.zeros(logits_neighbors.shape[0],
                             dtype=torch.long,
                             device=logits_neighbors.device)
    ...
    def forward(self, q, k_center, k_neighbors, queue, valid_neighbors_count):
        ...
        # Neighbor loss
        loss_neighbors = self.loss_neighbor(q, k_neighbors, queue,
